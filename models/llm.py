"""LLM implementation using Ollama (OpenAI-compatible API)."""
from __future__ import annotations

import os
from typing import Any, NotGivenOr
import httpx
import openai
...
class OllamaLLM(llm.LLM):
    ...
    def __init__(
        ...
        api_key: str = "NULL",  # Ollama doesn't need API key, but OpenAI client requires it
        ...
        Args:
            ...
            api_key: API key (not needed for Ollama, but required by OpenAI client)
            model: Model name (e.g., qwen2.5:7b, llama3:8b)
        ...
        timeout = httpx.Timeout(
            connect=15.0,
            read=60.0,  # Longer read timeout for local LLM
            write=5.0,
            pool=5.0
        )
...
class OllamaLLMStream(llm.LLMStream):
    ...
    def _parse_choice(self, id: str, choice: Choice) -> llm.ChatChunk | None:
        """Parse a choice from the stream."""
        delta = choice.delta
